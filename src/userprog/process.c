...
#ifdef VM
  frame_table_init(&(cur->frame_table));
  suppage_table_init(&(cur->suppage_table));
#endif
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      file_seek (file,total_read_bytes);
      suppage_insert(&thread_current()->suppage_table, upage, file, file_tell(file),page_read_bytes,true,writable);
...
setup_stack (void **esp) 
{
  uint8_t *kpage;
  bool success = false;
  kpage = frame_get_page (&thread_current()->frame_table,(uint8_t*)PHYS_BASE - PGSIZE, PAL_USER | PAL_ZERO,true);
  if (kpage != NULL) 
    {
      //success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
        ...
        *esp = PHYS_BASE;
     // else
       //   frame_free_page (&thread_current()->frame_table,((uint8_t*) PHYS_BASE) - PGSIZE);
    }
  return success;
}
